...
from pybel.examples import sialic_acid_example, sialic_acid_graph
...
interleukin_23 = named_complex_abundance(
    namespace='GOCC',
...
class TestNetworkCache(BelReconstitutionMixin, FleetingTemporaryCacheMixin):
    ...
    def test_reload(self, mock_get):
        ...
        self.assertEqual(sialic_acid_graph.name, network.name)
        self.assertEqual(sialic_acid_graph.version, network.version)
        self.assertEqual(sialic_acid_graph.description, network.description)

        reconstituted = self.manager.get_graph_by_name_version(sialic_acid_graph.name, sialic_acid_graph.version)
        self.assertEqual(sialic_acid_graph.nodes(), reconstituted.nodes())
...
class TestQueryEdgeStore(TemporaryCacheClsMixin, BelReconstitutionMixin):
    """Tests that the cache can be queried"""

    @classmethod
    def setUpClass(cls):
        super(TestQueryEdgeStore, cls).setUpClass()

        cls.graph = sialic_acid_graph.copy()
    ...
    def test_citations(self):
        ...
        citation_references = {sialic_acid_example.citation[CITATION_REFERENCE]}
    ...
    def test_authors(self):
        authors = set(sialic_acid_example.citation[CITATION_AUTHORS].split('|'))
    ...
    def test_evidences(self):
        ...
        evidences_texts = {sialic_acid_example.evidence_1, sialic_acid_example.evidence_2}
    ...
    def test_nodes(self):
        self.assertEqual(sialic_acid_graph.number_of_nodes(), self.manager.count_nodes())

    def test_edges(self):
        self.assertEqual(sialic_acid_graph.number_of_edges(), self.manager.count_edges())
...
class TestAddNodeFromData(unittest.TestCase):
    ...
    def test_single_variant(self):
        node = gene(namespace='HGNC', name='AKT1', variants=[hgvs('p.Phe508del')])
    ...
    def test_multiple_variants(self):
        node = gene(namespace='HGNC', name='AKT', variants=[hgvs('p.Phe508del'), hgvs('p.Phe509del')])
    ...
    def test_composite(self):
        il23 = named_complex_abundance(namespace='GOCC', name='interleukin-23 complex')
        il6 = hgnc('IL6')
        ...
        self.assertIn(il6, self.graph[node])
        keys = list(self.graph[node][il6])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_COMPONENT, self.graph[node][il6][keys[0]][RELATION])

        self.assertIn(il23, self.graph[node])
        keys = list(self.graph[node][il23])
        self.assertEqual(1, len(keys))
    ...
    def test_reaction(self):
        ...
        self.assertIn(oxygen, self.graph)

        self.assertEqual(4, self.graph.number_of_nodes())
        self.assertEqual(3, self.graph.number_of_edges())

        self.assertIn(superoxide, self.graph[reaction_1])
        keys = list(self.graph[reaction_1][superoxide])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_REACTANT, self.graph[reaction_1][superoxide][keys[0]][RELATION])

        self.assertIn(hydrogen_peroxide, self.graph[reaction_1])
        keys = list(self.graph[reaction_1][hydrogen_peroxide])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_PRODUCT,
                         self.graph[reaction_1][hydrogen_peroxide][keys[0]][RELATION])

        self.assertIn(oxygen, self.graph[reaction_1])
        keys = list(self.graph[reaction_1][oxygen])
        self.assertEqual(1, len(keys))
    ...
    def test_complex(self):
        ...
        self.assertIn(fos, self.graph[ap1_complex])
        node_fos_edge_keys = list(self.graph[ap1_complex][fos])
        self.assertEqual(1, len(node_fos_edge_keys))
        self.assertEqual(HAS_COMPONENT, self.graph[ap1_complex][fos][node_fos_edge_keys[0]][RELATION])

        self.assertIn(jun, self.graph[ap1_complex])
        node_jun_edge_keys = list(self.graph[ap1_complex][jun])
        self.assertEqual(1, len(node_fos_edge_keys))
        self.assertEqual(HAS_COMPONENT, self.graph[ap1_complex][jun][node_jun_edge_keys[0]][RELATION])
    ...
    def test_dimer_complex(self):
        ...
        self.assertIn(egfr, self.graph[egfr_dimer])
        keys = list(self.graph[egfr_dimer][egfr])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_COMPONENT, self.graph[egfr_dimer][egfr][keys[0]][RELATION])
    ...
    def test_nested_complex(self):
        ...
        self.assertIn(fos, self.graph)
        self.assertIn(jun, self.graph)
        self.assertIn(e2f4, self.graph)
        self.assertIn(ap1_complex, self.graph)
        self.assertEqual(4, self.graph.number_of_edges())

        self.assertIn(fos, self.graph[ap1_complex])
        keys = list(self.graph[ap1_complex][fos])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_COMPONENT, self.graph[ap1_complex][fos][keys[0]][RELATION])

        self.assertIn(jun, self.graph[ap1_complex])
        keys = list(self.graph[ap1_complex][jun])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_COMPONENT, self.graph[ap1_complex][jun][keys[0]][RELATION])

        self.assertIn(ap1_complex, self.graph[bound_ap1_e2f4])
        keys = list(self.graph[bound_ap1_e2f4][ap1_complex])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_COMPONENT,
                         self.graph[bound_ap1_e2f4][ap1_complex][keys[0]][RELATION])

        self.assertIn(e2f4, self.graph[bound_ap1_e2f4])
        keys = list(self.graph[bound_ap1_e2f4][e2f4])
        self.assertEqual(1, len(keys))
        self.assertEqual(HAS_COMPONENT, self.graph[bound_ap1_e2f4][e2f4][keys[0]][RELATION])
...
class TestReconstituteNodeTuples(TemporaryCacheMixin):
    ...
    def test_fusion_specified(self, mock):
        ...
        self.help_reconstitute(tmprss2_erg_gene_fusion, namespaces, 1, 0)
    ...
    def test_composite(self, mock):
        ...
        self.help_reconstitute(interleukin_23_il6_composite, namespaces, number_nodes=3, number_edges=2)

    @mock_bel_resources
    def test_reaction(self, mock):
        namespaces = {'CHEBI': ['superoxide', 'hydrogen peroxide', 'oxygen']}
        self.help_reconstitute(reaction_1, namespaces, 4, 3)
