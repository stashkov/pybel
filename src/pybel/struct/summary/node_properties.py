...
from ..filters import get_nodes
from ..filters.edge_predicates import is_causal_relation
from ..filters.node_predicates import (
    has_activity, has_variant, is_causal_central, is_causal_sink, is_causal_source, is_degraded, is_translocated,
)
...
def get_causal_out_edges(graph, nbunch):
    ...
    :rtype: set[tuple[BaseEntity,BaseEntity]]
    """
    return {
        (u, v)
        for u, v, data in graph.out_edges(nbunch, data=True)
        if is_causal_relation(data)
    }


def get_causal_in_edges(graph, nbunch):
    ...
    return {
        (u, v)
        for u, v, data in graph.in_edges(nbunch, data=True)
        if is_causal_relation(data)
    }


def get_causal_source_nodes(graph, func):
    ...
    :param pybel.BELGraph graph: A BEL graph
    :param str func: The BEL function to filter by
    ...
    return {
        ...
        if data[FUNCTION] == func and is_causal_source(graph, node)
    }


def get_causal_central_nodes(graph, func):
    ...
    return {
        node
        for node, data in graph.nodes(data=True)
        if data[FUNCTION] == func and is_causal_central(graph, node)
    }


def get_causal_sink_nodes(graph, func):
    ...
    return {
        node
        for node, data in graph.nodes(data=True)
        if data[FUNCTION] == func and is_causal_sink(graph, node)
    }
